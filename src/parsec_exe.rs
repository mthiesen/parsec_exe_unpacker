...
pub fn parse_header<R: Read>(mut reader: R) -> Result<Info> {
    let header_data = {
        ...
        reader.read_exact(&mut buffer).chain_err(|| "Unable to read exe header.")?;
        buffer
    };
...
mod test {
    ...
    fn parse_correctly() {
        const HEADER: [u8; SIZE] = [
            0x4d, 0x5a, 0xca, 0x00, 0x9b, 0x00, 0x00, 0x00, 0x02, 0x00, 0x04, 0x13, 0xff, 0xff, 0x4b, 0x13,
            0x00, 0x01, 0x00, 0x00, 0xd0, 0x01, 0x29, 0x13, 0x1c, 0x00, 0x00, 0x00, 0x50, 0x52, 0x53, 0x43];
    ...
    fn exe_header_signature_missing() {
        const HEADER: [u8; SIZE] = [
            0x00, 0x00, 0xca, 0x00, 0x9b, 0x00, 0x00, 0x00, 0x02, 0x00, 0x04, 0x13, 0xff, 0xff, 0x4b, 0x13,
            0x00, 0x01, 0x00, 0x00, 0xd0, 0x01, 0x29, 0x13, 0x1c, 0x00, 0x00, 0x00, 0x50, 0x52, 0x53, 0x43];
    ...
    fn parsec_signature_missing() {
        const HEADER: [u8; SIZE] = [
            0x4d, 0x5a, 0xca, 0x00, 0x9b, 0x00, 0x00, 0x00, 0x02, 0x00, 0x04, 0x13, 0xff, 0xff, 0x4b, 0x13,
            0x00, 0x01, 0x00, 0x00, 0xd0, 0x01, 0x29, 0x13, 0x1c, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00];
        assert!(parse_header(&HEADER[..]).is_err());
    }
    ...
    fn has_relocation_table() {
        const HEADER: [u8; SIZE] = [
            0x4d, 0x5a, 0xca, 0x00, 0x9b, 0x00, 0x05, 0x00, 0x02, 0x00, 0x04, 0x13, 0xff, 0xff, 0x4b, 0x13,
            0x00, 0x01, 0x00, 0x00, 0xd0, 0x01, 0x29, 0x13, 0x1c, 0x00, 0x00, 0x00, 0x50, 0x52, 0x53, 0x43];
        assert!(parse_header(&HEADER[..]).is_err());
    }
    ...
    fn header_wrong_size() {
        const HEADER: [u8; SIZE] = [
            0x4d, 0x5a, 0xca, 0x00, 0x9b, 0x00, 0x00, 0x00, 0x01, 0x00, 0x04, 0x13, 0xff, 0xff, 0x4b, 0x13,
            0x00, 0x01, 0x00, 0x00, 0xd0, 0x01, 0x29, 0x13, 0x1c, 0x00, 0x00, 0x00, 0x50, 0x52, 0x53, 0x43];
        assert!(parse_header(&HEADER[..]).is_err());
    }
    ...
    fn invalid_page_count() {
        const HEADER: [u8; SIZE] = [
            0x4d, 0x5a, 0xca, 0x00, 0x00, 0x00, 0x00, 0x00, 0x02, 0x00, 0x04, 0x13, 0xff, 0xff, 0x4b, 0x13,
            0x00, 0x01, 0x00, 0x00, 0xd0, 0x01, 0x29, 0x13, 0x1c, 0x00, 0x00, 0x00, 0x50, 0x52, 0x53, 0x43];
        assert!(parse_header(&HEADER[..]).is_err());
    }
    ...
    fn initial_stack_ptr_out_of_bounds() {
        const HEADER: [u8; SIZE] = [
            0x4d, 0x5a, 0xca, 0x00, 0x00, 0x00, 0x00, 0x00, 0x02, 0x00, 0x04, 0x13, 0xff, 0xff, 0xff, 0xff,
            0x00, 0x01, 0x00, 0x00, 0xd0, 0x01, 0x29, 0x13, 0x1c, 0x00, 0x00, 0x00, 0x50, 0x52, 0x53, 0x43];
        assert!(parse_header(&HEADER[..]).is_err());
    }
    ...
    fn entry_point_out_of_bounds() {
        const HEADER: [u8; SIZE] = [
            0x4d, 0x5a, 0xca, 0x00, 0x00, 0x00, 0x00, 0x00, 0x02, 0x00, 0x04, 0x13, 0xff, 0xff, 0x4b, 0x13,
            0x00, 0x01, 0x00, 0x00, 0xd0, 0x01, 0xff, 0xff, 0x1c, 0x00, 0x00, 0x00, 0x50, 0x52, 0x53, 0x43];
        assert!(parse_header(&HEADER[..]).is_err());
    }
